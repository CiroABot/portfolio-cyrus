...
import { cinebeijoca, ucbProductions, ucbProducer, workshops, oficina } from '../content/projects';
...
const Projects: React.FC<ProjectsProps> = ({ onOpenZine, onPlayVignette }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`w-full max-w-[1300px] mx-auto px-5 md:px-10 lg:pr-[200px] xl:pr-[240px] relative z-10 ${isExpanded ? 'block' : 'hidden md:block'}`}>
                    ...
                    <div className="p-8 md:p-10 flex flex-col justify-between border-b-3 xl:border-b-0 xl:border-r-3 border-black relative overflow-hidden">
                        ...
                        <div className="mt-auto bg-[#f4f4f4] p-4 border-2 border-black shadow-[4px_4px_0px_#000]">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div><strong className="block font-mono font-bold uppercase text-navy text-xs mb-1">★ {t.support_label}</strong><p className="font-mono font-bold text-sm tracking-tight leading-tight">{cinebeijoca.support}</p></div>
                            </div>
                        </div>
                    </div>

                    <div className="flex flex-col bg-[#f0f0f0]">
                        <button onClick={() => onPlayVignette(cinebeijoca.vignetteUrl)} className="flex-1 min-h-[200px] relative border-b-3 border-black group/btn overflow-hidden interactive-target">
            ...
            <div className="relative group">
                ...
                <div className="relative z-10 bg-black text-white border-3 border-black p-6 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {ucbProductions.map((monitor, i) => (
                            <div key={monitor.title} className="bg-[#111] border-3 border-white/20 flex flex-col shadow-[5px_5px_0px_#222] group/monitor">
                                 <div className="relative aspect-video bg-black border-b-2 border-white/20 overflow-hidden group/screen">
                                     <div className="absolute top-2 left-2 font-mono text-[10px] text-black bg-yellow px-1 border border-white font-bold z-20">CAM_0{i + 1}</div>
                                     <Image src={monitor.image} alt={monitor.title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover opacity-60 group-hover/screen:opacity-100 transition-opacity grayscale group-hover/screen:grayscale-0" />
                                 </div>
                                 <div className="p-4 flex flex-col h-full relative">
                                     <strong className="font-title italic text-yellow text-xl uppercase leading-none mb-2 border-b border-white/20 pb-2">{monitor.title}</strong>
                                     <p className="font-mono font-bold text-xs text-teal mb-4 flex-grow uppercase">{localize(monitor.role, lang)}</p>
                                     <div className="text-[10px] font-mono font-bold text-rose border-t border-white/20 pt-2 mb-3 uppercase">
                                         {monitor.credits}
                                     </div>
                                     <button
                                         onClick={() => onPlayVignette(monitor.videoUrl)}
            ...
            {/* CHANNEL 03 */}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white border-3 border-black p-6 shadow-[8px_8px_0px_#d17bac] relative overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:-rotate-1 hover:shadow-[12px_12px_0px_#d17bac] z-10 hover:z-20">
                    ...
                    <div className="mt-auto">
                        <div className="text-xs font-mono text-gray-500 uppercase">
                             <strong>{t.work_prod_label}</strong> {workshops.aiEthicsProduction}
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white border-3 border-black p-6 shadow-[8px_8px_0px_#7bc8d1] relative overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:rotate-1 hover:shadow-[12px_12px_0px_#7bc8d1] z-10 hover:z-20">
                    ...
                    <div className="mt-auto">
                         <div className="text-xs font-mono text-gray-500 uppercase">
                             <strong>{t.work_prod_label}</strong> {workshops.culturaProduction}
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="p-8 md:p-10 flex flex-col justify-between relative">
                         ...
                         <div className="mt-auto bg-[#f4f4f4] p-4 border-2 border-black shadow-[4px_4px_0px_#000]">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div><span className="block font-mono font-bold uppercase text-teal text-xs mb-1">★ {t.support_label}</span><span className="font-mono font-bold text-sm tracking-tight leading-tight">{oficina.support}</span></div>
                            </div>
                         </div>
                    </div>
                </div>
